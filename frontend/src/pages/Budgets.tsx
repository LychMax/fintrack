...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 relative max-h-[90vh] overflow-y-auto"
...
function BudgetCard({
  ...
  const periodLabel =
    status.periodType === PeriodType.MONTHLY ? "на месяц" : "на неделю";

  return (
    <div
      className="rounded-2xl p-4 lg:p-5 transition-all hover:border-primary/30"
      style={{
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3 lg:mb-4">
        <div className="min-w-0 mr-2">
          <div className="flex items-center gap-2">
            ...
            {!status.exceeded && status.nearLimit && (
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm" style={{ color: barColor }}>
          {symbol}{" "}
          {status.spentAmount.toLocaleString("ru-RU", {
            minimumFractionDigits: 2,
          })}{" "}
          /{" "}
          {symbol}{" "}
          {status.budgetAmount.toLocaleString("ru-RU", {
            minimumFractionDigits: 2,
          })}
        </span>
        <span
      ...
      <p className="text-xs" style={{ color: barColor }}>
        {status.exceeded
          ? `Превышен на ${symbol} ${(
              status.spentAmount - status.budgetAmount
            ).toLocaleString("ru-RU", { minimumFractionDigits: 2 })}`
          : `Осталось ${symbol} ${status.remainingAmount.toLocaleString(
              "ru-RU",
              { minimumFractionDigits: 2 }
            )}`}
      </p>
...
export default function Budgets() {
  ...
  const [editingStatus, setEditingStatus] = useState<BudgetStatusDto | null>(
    null
  );
  ...
  const monthlyBudgets = statuses.filter(
    (s) => s.periodType === PeriodType.MONTHLY
  );
  const weeklyBudgets = statuses.filter(
    (s) => s.periodType === PeriodType.WEEKLY
  );
    ...
    <div className="space-y-5 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {statuses.length} активных бюджетов
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      >
        ...
        <p className="text-sm text-slate-400">
          Бюджеты автоматически сбрасываются в начале каждого периода. Все
          суммы конвертируются в вашу основную валюту.
        </p>
      </div>
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-red-400">
                  {exceeded} превышен{exceeded > 1 ? "о" : ""}
                </p>
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-amber-400">
                  {nearLimit} близк{nearLimit > 1 ? "о" : "о"} к лимиту
                </p>
      ...
      {isLoading ? (
        ...
        <div
          className="text-center py-20 lg:py-24 rounded-2xl"
          style={{
            background: "rgba(30,41,59,0.4)",
            border: "1px dashed rgba(51,65,85,0.6)",
          }}
        >
          <Wallet className="w-14 h-14 lg:w-16 lg:h-16 text-muted-foreground mx-auto mb-4 opacity-40" />
          <h3 className="text-lg lg:text-xl font-semibold text-foreground mb-2">
            Бюджеты не установлены
          </h3>
          <p className="text-muted-foreground text-sm mb-6">
            Установите лимиты по категориям и следите за тратами
          </p>
          <button
            onClick={openCreate}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[
                { v: PeriodType.MONTHLY, label: "Месяц" },
                { v: PeriodType.WEEKLY, label: "Неделя" },
              ].map(({ v, label }) => (
                <button
                  key={v}
                  type="button"
                  onClick={() => setValue("periodType", v)}
