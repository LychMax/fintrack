...
import {
  ...
  AlertTriangle, ChevronRight, Plus, Receipt
} from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-3xl p-8 relative overflow-hidden"
        style={{ background: "linear-gradient(135deg, rgba(236,72,153,0.15) 0%, rgba(168,85,247,0.15) 100%)", border: "1px solid rgba(236,72,153,0.2)" }}>
        <div className="absolute inset-0" style={{
          backgroundImage: "radial-gradient(circle at 80% 20%, rgba(168,85,247,0.1) 0%, transparent 50%)"
        }} />
        <div className="relative z-10">
          <p className="text-sm text-slate-400 mb-2 font-medium">Общий баланс (за всё время)</p>
          ...
          <div className="flex gap-8">
            <div>
              ...
              <p className="text-xl font-bold text-green-400">{fmt(allTimeReport?.totalIncome ?? 0)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400 mb-1 flex items-center gap-1">
                <TrendingDown className="w-3 h-3 text-red-400" /> Все расходы
              </p>
              <p className="text-xl font-bold text-red-400">{fmt(allTimeReport?.totalExpense ?? 0)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            value={fmt((monthReport?.totalIncome ?? 0) - (monthReport?.totalExpense ?? 0))}
            color="#EC4899"
            icon={Wallet}
          />
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="rounded-2xl p-6" style={{ background: "rgba(30,41,59,0.6)", border: "1px solid rgba(51,65,85,0.5)" }}>
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to="/budgets" className="text-sm text-primary hover:text-primary/80 flex items-center gap-1 transition-colors">
          ...
          {budgetStatuses.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground text-sm mb-4">Бюджеты не установлены</p>
              <Link to="/budgets"
            ...
            <div className="space-y-4">
              {budgetStatuses.slice(0, 5).map((b) => {
                ...
                const barColor = b.exceeded ? "#ef4444" : b.nearLimit ? "#f59e0b" : "#22c55e";
                  ...
                  <div key={b.id}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        {b.exceeded && <AlertTriangle className="w-3.5 h-3.5 text-red-400" />}
                        {!b.exceeded && b.nearLimit && <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />}
                        <span className="text-sm font-medium text-foreground">{b.categoryName}</span>
                        <span className="text-xs text-muted-foreground">
                          {b.periodType === "MONTHLY" ? "/ месяц" : "/ неделю"}
                        </span>
                      </div>
                    ...
                    <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(51,65,85,0.8)" }}>
                      <div className="h-full rounded-full transition-all duration-500"
                        style={{ width: `${pct}%`, background: barColor }} />
                    </div>
                    <div className="flex justify-between mt-1">
                      <span className="text-xs text-muted-foreground">
                        {symbol} {b.spentAmount.toFixed(0)} / {symbol} {b.budgetAmount.toFixed(0)}
                      </span>
                      <span className="text-xs" style={{ color: barColor }}>
                        {b.exceeded
                          ? `Превышен на ${symbol} ${(b.spentAmount - b.budgetAmount).toFixed(0)}`
                          : `Осталось ${symbol} ${b.remainingAmount.toFixed(0)}`}
                      </span>
                    </div>
                  </div>
                );
        ...
        {/* Recent transactions — ИСПРАВЛЕНО */}
        <div className="rounded-2xl p-6" style={{ background: "rgba(30,41,59,0.6)", border: "1px solid rgba(51,65,85,0.5)" }}>
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-semibold text-foreground">Последние операции</h3>
            <Link to="/transactions" className="text-sm text-primary hover:text-primary/80 flex items-center gap-1 transition-colors">
              Все <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
                ...
                return (
                  <div key={t.id} className="flex items-center gap-3 py-2 border-b border-border/20 last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{t.category.name}</p>
                      {t.description && (
                    ...
                    {/* ИСПРАВЛЕНО: используем mainSymbol */}
                    <p className="text-sm font-bold flex-shrink-0" style={{ color }}>
                      {sign} {mainSymbol} {t.amount.toLocaleString("ru-RU", { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                );
              })}
