...
import {
  TransactionType, TransactionDto, TransactionCreateDto,
  Currency, CurrencySymbol, CurrencyLabel,
} from "@/types";
import { format } from "date-fns";
import { ru } from "date-fns/locale";
import {
  Plus, Search, Trash2, Edit, ChevronLeft, ChevronRight,
  X, ArrowUpRight, ArrowDownRight, Filter, Loader2, Check,
} from "lucide-react";
...
function Modal({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}
      ...
      <div className="w-full max-w-lg rounded-2xl p-6 relative" onClick={(e) => e.stopPropagation()}
...
export default function Transactions() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters bar */}
      <div className="rounded-2xl p-4" style={{ background: "rgba(30,41,59,0.6)", border: "1px solid rgba(51,65,85,0.5)" }}>
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${hasFilters ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}
            style={{ background: hasFilters ? "rgba(236,72,153,0.1)" : "rgba(51,65,85,0.4)", border: hasFilters ? "1px solid rgba(236,72,153,0.3)" : "1px solid transparent" }}
          >
            <Filter className="w-4 h-4" /> Фильтры {hasFilters && <span className="w-2 h-2 rounded-full bg-primary" />}
          </button>

          {/* Type pills */}
          <div className="flex gap-2">
            {[{ v: "" as const, label: "Все" }, { v: TransactionType.INCOME, label: "Доходы" }, { v: TransactionType.EXPENSE, label: "Расходы" }].map(({ v, label }) => (
              <button key={v} onClick={() => { setFilterType(v); setPage(0); }}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterType === v ? "text-white" : "text-muted-foreground hover:text-foreground"}`}
                style={{ background: filterType === v ? "rgba(236,72,153,0.2)" : "rgba(51,65,85,0.4)", border: filterType === v ? "1px solid rgba(236,72,153,0.4)" : "1px solid transparent" }}
              >
                {label}
              </button>
            ))}
          ...
          {hasFilters && (
            <button onClick={() => { setFilterFrom(""); setFilterTo(""); setFilterType(""); setFilterCategoryId(null); setPage(0); }}
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-border/30 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">От даты</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">До даты</label>
            ...
            <div>
              ...
              <select value={filterCategoryId ?? ""}
                onChange={(e) => { setFilterCategoryId(e.target.value ? Number(e.target.value) : null); setPage(0); }}
                className="w-full px-3 py-2 rounded-xl text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/50"
                style={{ background: "rgba(15,23,42,0.8)", border: "1px solid rgba(51,65,85,0.8)" }}>
      ...
      {/* Table */}
      <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(30,41,59,0.6)", border: "1px solid rgba(51,65,85,0.5)" }}>
        {isLoading ? (
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(51,65,85,0.5)" }}>
                  {["Дата", "Категория", "Описание", "Тип", "Сумма", "Валюта", ""].map((h) => (
              ...
              <tbody>
                {transactions.map((t, i) => {
                  ...
                  const sign = isIncome ? "+" : "-";
                  const txSymbol = CurrencySymbol[t.currency as Currency] || "Br";
                  return (
                    <tr key={t.id}
                      className="hover:bg-white/[0.02] transition-colors"
                      style={{ borderBottom: i < transactions.length - 1 ? "1px solid rgba(51,65,85,0.3)" : "none" }}>
                      <td className="px-5 py-4 text-sm text-muted-foreground whitespace-nowrap">
                        {format(new Date(t.date), "dd MMM yyyy", { locale: ru })}
                        <div className="text-xs opacity-60">{format(new Date(t.date), "HH:mm")}</div>
                      </td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        <div className="flex gap-2 mb-5">
          {[{ v: TransactionType.INCOME, label: "Доход", color: "#22c55e" }, { v: TransactionType.EXPENSE, label: "Расход", color: "#ef4444" }].map(({ v, label, color }) => (
            <button key={v} type="button" onClick={() => setValue("type", v)}
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all"
              style={{
                background: watchedType === v ? `${color}20` : "rgba(51,65,85,0.4)",
                border: watchedType === v ? `1.5px solid ${color}60` : "1.5px solid transparent",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1.5 block">Дата и время</label>
          ...
          <div>
            ...
            <input {...register("description")} placeholder="Заметка..."
              className="w-full px-4 py-2.5 rounded-xl text-foreground placeholder-slate-500 outline-none focus:ring-2 focus:ring-primary/50 text-sm"
              style={{ background: "rgba(15,23,42,0.8)", border: "1px solid rgba(51,65,85,0.8)" }} />
          </div>
      ...
      <Modal open={deleteConfirm !== null} onClose={() => setDeleteConfirm(null)}>
        <h2 className="text-xl font-bold text-foreground mb-3">Удалить транзакцию?</h2>
