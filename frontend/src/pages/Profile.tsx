...
function SuccessToast({
  ...
  return (
    <div
      className="fixed bottom-4 left-4 right-4 sm:left-auto sm:right-6 sm:bottom-6 sm:w-auto z-50 flex items-center gap-3 px-5 py-3 rounded-xl shadow-2xl"
      style={{
        background: "#1e293b",
    ...
    >
      ...
      <button
        onClick={onClose}
        className="text-muted-foreground hover:text-foreground ml-2 text-xs flex-shrink-0"
      >
        ✕
      </button>
    </div>
...
export default function Profile() {
  ...
  const profileForm = useForm<ProfileForm>({
    ...
    values: {
      username: profile?.username || displayUsername,
  ...
  const onProfileSubmit = async (data: ProfileForm) => {
    setProfileError("");
    const payload: any = {};
    if (data.username && data.username !== displayUsername)
  ...
  return (
    <div className="space-y-5 lg:space-y-8 max-w-3xl">
      {/* Header / Avatar */}
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 lg:gap-6">
        <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white text-3xl lg:text-4xl font-bold shadow-xl flex-shrink-0">
          {avatarLetter}
        </div>
        <div className="text-center sm:text-left">
          ...
          <p className="text-muted-foreground mt-1 text-sm">{displayEmail}</p>
          <span
          ...
          >
            <ShieldCheck className="w-3 h-3" /> Валюта: {displayCurrency}
          </span>
        </div>
      </div>
      ...
      <Section title="Личные данные" icon={User}>
        ...
        >
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-white text-sm transition-all hover:opacity-90 disabled:opacity-60 w-full sm:w-auto"
      ...
      <Section title="Основная валюта" icon={Wallet}>
        <p className="text-sm text-muted-foreground mb-4">
          Все суммы будут отображаться в выбранной валюте с автоматической
          конвертацией.
        </p>
          ...
          {Object.entries(CurrencyLabel).map(([code, label]) => {
            ...
            return (
              <button
                ...
                disabled={updateProfile.isPending}
                className="py-3 px-3 lg:px-4 rounded-xl text-sm font-medium transition-all flex flex-col items-center gap-1 disabled:opacity-60"
                style={{
              ...
              >
                <span className="text-base lg:text-lg font-bold">{code}</span>
